...
namespace Ogre {
	...
	//-----------------------------------------------------------------------
	DataStreamPtr LockedZipArchive::open(const String& filename, bool readOnly ) const
	{
		...
		if( idx != (-1) )
		{
			...
			if(m_zipArchive->GetFileInfo(info,(WORD)idx))
			{
				...
				return DataStreamPtr(OGRE_NEW LockedZipDataStream(filename, pmf, static_cast<size_t>(info.m_uUncomprSize)));
			}
		}
	...
	{
		return m_zipArchive->FindFile(filename.c_str()) != ZIP_FILE_INDEX_NOT_FOUND;
	}


	void LockedZipArchive::calculatePassword()
	{
		strcpy(password,"10200805");
	}
